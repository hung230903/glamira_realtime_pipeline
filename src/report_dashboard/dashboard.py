...
with col1:
    ...
    query_top_products = """
                         ...
                         ORDER BY view_count DESC
                             LIMIT 10 \
                         """
    df_top_products = load_data(query_top_products, params={"selected_date": selected_date})
...
with col2:
    ...
    query_top_countries = """
                          ...
                          ORDER BY view_count DESC
                              LIMIT 10 \
                          """
    df_top_countries = load_data(query_top_countries, params={"selected_date": selected_date})
...
query_top_referrers = """
                      ...
                      ORDER BY view_count DESC
                          LIMIT 5 \
                      """
df_top_referrers = load_data(query_top_referrers, params={"selected_date": selected_date})
...
country_list = df_all_countries['country_name'].tolist() if not df_all_countries.empty else []
...
if country_list:
    ...
    if selected_country:
        query_country_stores = """
                               ...
                               ORDER BY view_count DESC \
                               """
        df_country_stores = load_data(query_country_stores,
        ...
        if not df_country_stores.empty:
            ...
            with col_chart_4:
                ...
                fig_store = px.bar(
                    df_country_stores, x='store_id', y='view_count',
            ...
            st.info(f"No view data found for stores in {selected_country} on this date.")
else:
...
df_recent_products = load_data(
    "SELECT DISTINCT product_id FROM fact_product_views WHERE DATE(time_stamp) = :selected_date LIMIT 100",
    params={"selected_date": selected_date})
product_suggestions = df_recent_products['product_id'].tolist() if not df_recent_products.empty else []

if product_suggestions:
    product_id_input = st.selectbox("Select or type a Product ID:", [""] + product_suggestions)
else:
    product_id_input = st.text_input("Enter Product ID:", "")

if product_id_input:
    query_product_hourly = """
                           SELECT EXTRACT(HOUR FROM time_stamp) as hour, COUNT(*) as view_count
                           FROM fact_product_views
                           WHERE product_id = :product_id AND DATE (time_stamp) = :selected_date
                           GROUP BY hour
                           ORDER BY hour \
                           """
    df_product_hourly = load_data(query_product_hourly,
    ...
    if not df_product_hourly.empty:
        ...
        st.metric(label=f"Total Views for **{product_id_input}**", value=int(total_views))

        col_chart_5, col_data_5 = st.columns([2, 1])
        with col_chart_5:
            # Use Plotly Area chart for a sleek hourly time-series view
            fig_hourly = px.area(
                df_product_hourly, x='hour', y='view_count',
                title='Hourly View Trend', markers=True,
...
if not df_browser_os.empty:
    ...
    with col_pie1:
        fig_browser = px.pie(df_browser, values='view_count', names='browser', title='Browser Distribution', hole=0.3)
        st.plotly_chart(fig_browser, use_container_width=True)

    with col_pie2:
        fig_os = px.pie(df_os, values='view_count', names='os', title='OS Distribution', hole=0.3)
